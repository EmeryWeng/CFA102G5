...
<link
	href="<%=request.getContextPath()%>/back_end/activity/css/act/addAct.css"
	rel="stylesheet">
...
<body>
			...
			<div class="col-lg-12">
				<div class="card">
					<div class="card-header">
						<h1 class="card-title" style="font-size: 2rem; color: green;">修改活動場次-表單</h1>
					</div>
					<div class="card-body">
						<div class="form-validation">
							<form class="needs-validation" method="post" action="<%=request.getContextPath()%>/activity/ActivitySession">
								<div class="row">
									<div class="col-xl-6">
										<div class="mb-3 row">
											<select name="actNoSelect" class="select">
												<c:forEach var="actVO" items="${actService.all}">
													<option value="${actVO.act_no}" ${updateActSession_actSessionVO.act_no == actVO.act_no ? 'selected':''}>${actVO.act_name}</option>
												</c:forEach>
											</select>
										</div>
										<div class="mb-3 row">
											<label class="col-lg-4 col-form-label" for="start_date">可開始報名日
										...
										<div class="mb-3 row">
											<label class="col-lg-4 col-form-label" for="end_date">報名最後截止日
												<span class="text-danger">*</span>
											</label>
											<div class="col-lg-6">
												<input type="text" class="form-control"
													name="actEndDate" id="act_end_date">
											</div>
										</div>
									</div>
									<div class="mb-3 row">
										<label class="col-lg-4 col-form-label actInstructionLabel"
											for="act_session_start_date"> 活動場次開始日期 <span
											class="text-danger">*</span>
										</label>
										<div class="col-lg-6">
											<input type="text" class="form-control"
												name="actSessionStartDate" id="act_session_start_date">
										</div>
									</div>
								</div>
								<div class="col-xl-6">
									<div class="mb-3 row">
										<label class="col-lg-4 col-form-label"
											for="act_session_start_date">活動場次開始時間 <span
											class="text-danger">*</span>
										</label>
										<div class="col-lg-6">
											<input type="text" class="form-control"
	...
	<script>
		...
		$('#act_start_date').datetimepicker({
			...
			step : 1,
			format : 'Y-m-d',
			value : '${updateActSession_actSessionVO.act_start_date}',
			minDate : new Date(),
			maxDate : '2021-12-31',
		});
		...
		$('#act_end_date').datetimepicker({
			...
			step : 1,
			format : 'Y-m-d',
			value : '${updateActSession_actSessionVO.act_end_date}',
			minDate : new Date(),
			maxDate : '2021-12-31',
		});
		...
		$('#act_session_start_date').datetimepicker({
			...
			step : 1,
			format : 'Y-m-d',
			value : '${updateActSession_actSessionVO.act_session_start_date}',
			minDate : new Date(),
			maxDate : '2021-12-31',
		});
		...
		$('#act_session_start_time').datetimepicker({
			...
			value : '${updateActSession_actSessionVO.act_session_start_time}', // value:   new Date(),
			minDate:'2021-12-31',
			maxDate:'2021-12-31',
